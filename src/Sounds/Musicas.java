...
//                mediaPlayer.play(linhas.get(index));
...
//    }
    ...
    public void reproduzEscolha(JList list, JProgressBar progressBar, JLabel tempoAtual, JLabel tempoFinal, JLabel songTitle ) {
        list.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                try {
                    Media media = new Media(new File(linhas.get(index)).toURI().toString()); // Converte o caminho para URI
                    mediaPlayer.stop();
                    mediaPlayer = new MediaPlayer(media);
                    mediaPlayer.play();
                    mediaPlayer.currentTimeProperty().addListener(new ChangeListener<Duration>() {
    ...
    public void musicaAnterior(JLabel songTitle, JProgressBar progressBar, JLabel tempoAtual, JLabel tempoFinal) {
        ...
        if (indiceAtual > 0) {
            indiceAtual--;
            trocarMusica(linhas.get(indiceAtual)); // Troca para a música anterior
